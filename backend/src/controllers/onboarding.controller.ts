...
export class OnboardingController {
  ...
  checkBureau = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!Object.values(KycOwnerType).includes(ownerType)) {
        res.status(400).json({
          success: false,
          message: 'Invalid ownerType',
        });
      ...
      // ownerType-specific guardrails
      if (ownerType === KycOwnerType.APPLICANT && coApplicantId) {
        res.status(400).json({
          success: false,
          message: 'coApplicantId not allowed for APPLICANT',
        });
        return;
      }
      ...
      if (ownerType === KycOwnerType.CO_APPLICANT && !coApplicantId) {
        res.status(400).json({
          success: false,
          message: 'coApplicantId is required for CO_APPLICANT',
        });
        return;
      }
