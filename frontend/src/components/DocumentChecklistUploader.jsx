...
const DocumentChecklistUploader = ({
  ...
  const handleMetaSave = async (docId, field, value) => {
    try {
      const response = await documentService.updateDocumentMetadata(docId, {
        [field]: value
      })
                    ...
                    <div key={doc.id} className="mt-4 p-3 bg-white rounded border">
                      ...
                      {!readOnly && (
                        <div className="grid grid-cols-3 gap-3 mt-3">
                          ...
                          <input
                            type="date"
                            value={currentEdit.expiryDate ?? doc.expiryDate?.split('T')[0] ?? ''}
                            onChange={(e) =>
                              handleMetaChange(doc.id, 'expiryDate', e.target.value)
                            }
                            onBlur={(e) =>
                              handleMetaSave(doc.id, 'expiryDate', e.target.value)
                            }
                            className="border rounded p-1 text-xs"
                          />
                          ...
                          <input
                            ...
                            value={currentEdit.rmRemarks ?? doc.rmRemarks ?? ''}
                            onChange={(e) =>
                              handleMetaChange(doc.id, 'rmRemarks', e.target.value)
                            }
                            onBlur={(e) =>
                              handleMetaSave(doc.id, 'rmRemarks', e.target.value)
                            }
                            className="border rounded p-1 text-xs"
                          />
