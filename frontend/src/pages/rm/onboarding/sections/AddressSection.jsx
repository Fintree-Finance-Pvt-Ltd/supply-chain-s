...
const AddressSection = ({ addresses, setAddresses, errors }) => {
  const add = () => {
    ...
    setAddresses((p) => [...p, { id: null, localKey, type: "", fullAddress: "", pincode: "", state: "", city: "" }]);
  };

  const update = (key, patch) => {
    setAddresses((p) => p.map((x) => (x.id === key || x.localKey === key ? { ...x, ...patch } : x)));
  };

  const remove = (key) => {
    setAddresses((p) => p.filter((x) => x.id !== key && x.localKey !== key));
  };
        ...
        <div className="space-y-4">
          {addresses.map((addr, index) => (
            <AddressForm
              ...
              index={index}
              data={addr}
              onChange={(key, data) => update(addr.id || addr.localKey, data)}
              onRemove={() => remove(addr.id || addr.localKey)}
              readOnly={false}
              errors={{
